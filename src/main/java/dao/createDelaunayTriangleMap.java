...
public class createDelaunayTriangleMap {
    ...
    public static DelaunayTriangle createSuperTriangle(List<Point> points) {
        ...
        Point SB = new Point(0, (longitudeMin + longitudeMax) / 2, (latitudeMax * 2 - latitudeMin));
        Point SA = new Point(0, xa, h);
        Point SC = new Point(0, xc, h);
        //A、C两点下移，调大超级三角形ABC
        Line superTriangleLine = new Line(SB, SA);
        ...
        return new DelaunayTriangle(SA, SB, SC);
    }

    //创建三角网
    public static void createDelanuaryTriangleMap(List<Point> points, List<DelaunayTriangle> temp, List<DelaunayTriangle> triangles, List<Edge> buffer) {
        //构建初始的三角网
        //从左到右遍历所有的点
        for (Point point : points) {
            ...
            buffer.clear();
            //顺序遍历所有temp中的三角形
            for (DelaunayTriangle triangle : temp) {
                //获取点和三角形外接圆的位置关系
